...
* Modified for writting into a layer by Sergio Montoro                        *
...
var treeMenuDocument;       // Handle to the menu layer.
...
var menuHTML;				        // Inner HTML of menu layer
...
function treeMenuDisplay() {
  ...
  treeMenuDocument = document.getElementById(treeMenuFrame);
  ...
  menuHTML += "<tr valign=top><td>";
  menuHTML += '<img src="../images/images/tree/menu_root.gif" align=left border=0 vspace=0 hspace=0>';
  menuHTML += '<font face="' + treeMenuFont + '" size=' + treeMenuFontSize + ' color="black">&nbsp;<b>' + treeMenuRoot + '</b>&nbsp;</font>';
  menuHTML += "</td></tr>";
  treeMenuListItems(treeMenu);
  menuHTML += "</table>";
  
  treeMenuDocument.innerHTML = menuHTML;
}
...
function treeMenuDisplayItem(item) {
  ...
  menuHTML += "<tr valign=top><td nowrap>";
  ...
  for (i = 0; i < treeMenuDepth; i++) {
    if (treeMenuBars[i]) {
      menuHTML += '<img src="' + treeMenuImgDir + 'menu_bar.gif" align=left border=0 vspace=0 hspace=0>';
      bars[i] = true;
    }
    else {
      menuHTML += '<img src="' + treeMenuImgDir + 'menu_spacer.gif" align=left border=0 vspace=0 hspace=0>';
      bars[i] = false;
    }
    if (item.index == treeMenuSelected)
      treeMenuScrollX += 18;
  ...
  if (!more) {
    ...
    menuHTML += '<img src="' + treeMenuImgDir + img + '" align=left border=0 vspace=0 hspace=0>';
  }

  // Write the start of the link tag so all of the following images and text
  // will be clickable.

  menuHTML += "<a style='color:black;text-decoration:none' href='#' onClick='" + item.clck + ";" + cmd + "'>";    
  ...
  if (item.submenu) {
    if (expanded)
      img = "folderopen_16x16.gif";
    else
      img = "folderclosed_16x16.gif";
  }
  else {
    if (item.icon != "")
      img = item.icon;
  ...
  if (treeMenuAltText)
    ...
    alt = '';
  menuHTML += '<img src="' + treeMenuImgDir + img + '"' + alt + ' align=left border=0 vspace=0 hspace=0>';

  // Write the link text and finish the link and table row.

  if (item.index == treeMenuSelected)
    menuHTML += '<font face="' + treeMenuFont + '" size=' + treeMenuFontSize + '>&nbsp;<span style="background-color:' + treeMenuHiBg + ';color:' + treeMenuHiFg + ';">' + item.text + '</span></font>';
  else
    menuHTML += '<font face="' + treeMenuFont + '" size=' + treeMenuFontSize + '>&nbsp;' + item.text + '</font>';
  menuHTML += '</a>';
  menuHTML += '</td></tr>';
...
function treeMenuScroll() {
  ...
  // Get a handle to the menu layer.

  var win = document.getElementById(treeMenuFrame);
