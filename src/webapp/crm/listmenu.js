...
function treeMenuDisplay() {
  ...
  treeMenuDocument.writeln('</head>');
  treeMenuDocument.writeln('<body background="' + treeMenuBackground + '" bgcolor="' + treeMenuBgColor + '" text="' + treeMenuFgColor + '" link="' + treeMenuFgColor + '" alink="' + treeMenuFgColor + '" vlink="' + treeMenuFgColor + '" onLoad="parent.treeMenuScroll();"><table border=0 cellpadding=0 cellspacing=0>');
  treeMenuDocument.write('<tr valign=top><td>');
  treeMenuDocument.write('<img src="../images/images/tree/menu_root.gif" align=left border=0 vspace=0 hspace=0>');
...
function treeMenuDisplayItem(item) {
  ...
  if (item.url != "")
    treeMenuDocument.write("<a href='" + item.url + "' target='msgsadmin' onClick='" + item.clck + ";" + cmd + "'>");    
  else
...
function selectCategory(guid) {
  for (var c=0; c<treeMenuCategories.length; c++) {
	 if (treeMenuCategories[c]==guid) {
	   var date = new Date ();
	   date.setTime (date.getTime() + (86400 * 1000 * treeMenuDays));
	   setCookie(treeMenuName + "-selected", String(c), date);
